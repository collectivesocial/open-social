...
import pg from "pg";
import { computeApiKeyLookup, hashApiKey } from "../src/lib/crypto";
...
describe("devnet smoke test", () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      `INSERT INTO apps (app_id, name, domain, creator_did, api_key, api_key_lookup, status, created_at, updated_at)
      ...
      [
        ...
        computeApiKeyLookup(TEST_API_KEY),
      ],
    );
  });
